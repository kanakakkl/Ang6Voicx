...
export class CategoriesComponent implements OnInit {
  ...
  isCategoryDisabled() {
    ...
    if (record && categoryFormDirty) {
      if (( record.length > 0 && record[0].CategoryName.trim() != categoryFormName.trim() && categoryFormName.length > 0)
        || ( record.length > 0 && record[0].Description.trim() != categoryFormDesc.trim() && categoryFormDesc.length > 0)
        || (this.updatecatImgname != undefined && record[0].ImageName != this.updatecatImgname)) {
        return false;
      } else {
        return true;
      }
  ...
  isSubcategoryDisabled() {
    ...
    if (record && subcategoryFormDirty) {
      if (( record.length > 0 && record[0].CategoryName != categoryName && categoryName.length > 0)
        || ( record.length > 0 && record[0].SubcategoryName.trim() != subcategoryFormName.trim() && subcategoryFormName.length > 0)
        || (record.length > 0 && record[0].Description.trim() != subcategoryFormDesc.trim() && subcategoryFormDesc.length > 0)
        || (this.updateSubcatImgname != undefined && record[0].ImageName != this.updateSubcatImgname)) {
        return false;
      } else {
        return true;
      }
  ...
  relatedSubcategories(category, val) {
    ...
    subCategoriesData.forEach(function (res) {
      if (res && res.CategoryName == categoryId) {
        res.Show = val;
  ...
  showSubCategory(subcategory, $event, i) {
    ...
    this.categories.forEach(function (res) {
      if (res && res.Id == subcategory.CategoryName) {
        if (res.Show == false) {
          ele = true;
        }
      }
